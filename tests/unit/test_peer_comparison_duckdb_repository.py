...
from app.models.peer_comparison_storage import StoredPeerComparisonDocument
...
from tests.unit.test_peer_comparison_gateway import COMPLETED_AT
...
class DuckDBPeerComparisonRepositoryTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_implements_port_and_creates_indexed_peer_table(self):
        with self.storage() as storage:
            self.assertIsInstance(storage, PeerComparisonRepository)
        ...
        try:
            ...
            indexes = {
                ...
                for row in connection.execute(
                    ...
                    "WHERE table_name = 'jarvis_peer_comparisons'"
                ).fetchall()
            }
        finally:
            connection.close()

        self.assertIn("jarvis_peer_comparisons", tables)
        self.assertTrue(
            {
                "jarvis_peer_comparison_scope_lookup",
                "jarvis_peer_comparison_fingerprint_lookup",
                "jarvis_peer_comparison_expiry_lookup",
            }.issubset(indexes)
        )

    def test_complete_json_and_metadata_survive_close_and_reopen(self):
        stored = build_entry()
        with self.storage() as storage:
            saved = storage.save_peer_comparison(stored)
            row = storage._connection.execute(
                "SELECT tenant_id, symbol, document_fingerprint, "
                "observation_date, metric_count, peer_count, payload_json "
                "FROM jarvis_peer_comparisons"
            ).fetchone()

        decoded = StoredPeerComparisonDocument.model_validate_json(row[6])
        with self.storage() as reopened:
            loaded = reopened.get_peer_comparison(
                stored.cache_key,
                scope=self.scope,
    ...
    def test_idempotent_save_and_conflict_are_transactional(self):
        ...
        with self.storage() as storage:
            first = storage.save_peer_comparison(stored)
            second = storage.save_peer_comparison(stored)
            with self.assertRaises(StorageConflictError):
                storage.save_peer_comparison(conflicting)
            loaded = storage.get_peer_comparison(
                stored.cache_key,
                scope=self.scope,
                as_of=self.clock.now,
            )
    ...
    def test_atomic_refresh_and_scope_failure_preserve_active_json(self):
        ...
        with self.storage() as storage:
            ...
            storage.save_peer_comparison(old)
            self.clock.now = refreshed.stored_at
            storage.replace_peer_comparison(refreshed, scope=self.scope)
            with self.assertRaises(StorageConflictError):
                storage.replace_peer_comparison(old, scope=self.scope)
            with self.assertRaises(StorageError):
                storage.replace_peer_comparison(refreshed, scope=other_scope)
            loaded = storage.get_peer_comparison(
                refreshed.cache_key,
                scope=self.scope,
                as_of=self.clock.now,
    ...
    def test_reference_reads_and_deletes_are_tenant_isolated(self):
        ...
        with self.storage() as storage:
            storage.save_peer_comparison(stored)
            self.assertIsNone(
                storage.get_peer_comparison_by_cache_entry_id(
            ...
            with self.assertRaises(ValueError):
                storage.get_peer_comparison_by_cache_entry_id(
                    "peer_comparison:not-a-fingerprint",
                    scope=self.scope,
                    as_of=self.clock.now,
                )
            self.assertFalse(
                storage.delete_peer_comparison(stored.cache_key, scope=other_scope)
            )
            self.assertTrue(
                storage.delete_peer_comparison(stored.cache_key, scope=self.scope)
            )
    ...
    def test_expiry_is_inclusive_and_physically_purged(self):
        ...
        with self.storage() as storage:
            ...
            row_count = storage._connection.execute(
                "SELECT count(*) FROM jarvis_peer_comparisons"
            ).fetchone()[0]
